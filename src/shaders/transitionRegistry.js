...
in vec2 v_uv;
// ALL THREE SAMPLERS ARE PREMULTIPLIED ALPHA. The compositor reconciles this
// before the pass runs (Renderer._compositeEdgeTransition): the accumulator
// already is, and the clip side — which the pipeline carries as STRAIGHT — is
// converted in one pass on the way in. This is not a detail, it is what makes a
// transition over transparent content correct at all: mixing, blurring or
// otherwise weighting straight RGBA drags a transparent pixel's *undefined*
// colour (for alpha video, codec garbage smeared across the matte by 4:2:0
// chroma subsampling) into the result wherever the two sides' alphas differ,
// and the present pass's rgb / a division then amplifies it into a blocky
// fringe along every alpha edge. Premultiplied, a transparent pixel
// contributes exactly zero colour. (No backticks in here — this whole header is
// a JS template literal; see the Code style note in CLAUDE.md.)
//
// What that means when writing a transition:
//   - mix() / weighted sums / blurs over the samplers are correct as written.
//   - a SOLID colour you introduce is vec4(rgb, 1.0) as always (a == 1, so the
//     two conventions agree) — but a TRANSLUCENT one must be premultiplied by
//     hand: vec4(rgb * a, a), never vec4(rgb, a).
//   - layering one sample over another is top + bot * (1.0 - top.a), not
//     mix(bot, top, top.a).
//   - adding emissive light (c.rgb += glow, then c.a = max(c.a, glow)) is
//     unchanged: an emissive term is already premultiplied by its own coverage.
...
vec4 transition(vec2 uv) {
  ...
  // Scaling premultiplied colour darkens without touching coverage, which is
  // exactly what a drop shadow should do to the layer underneath.
  bot.rgb *= 1.0 - shade * u_slide_shadow;

  // Premultiplied source-over. The straight-alpha form (mix by top.a, alpha =
  // max) double-counted the top layer's colour where it was translucent and
  // pulled the bottom layer's transparent regions in at full weight.
...
vec4 transition(vec2 uv) {
  ...
  // The gap colour is introduced translucent, so it has to be premultiplied by
  // hand — the samplers already are (see the header). vec4(rgb, a) here would
  // be over-bright by 1/a once the present pass divides the alpha back out.
  vec4 gap = vec4(u_slice_gap * u_slice_gap_alpha, u_slice_gap_alpha);
  return mix(gap, c, inside);
}
`,
  },
...
//
// KNOWN DIVERGENCE: a TRANSITION_FX node's inputs are ordinary graph FBOs, i.e.
// STRAIGHT alpha, whereas the edge compositor now hands the same bodies
// PREMULTIPLIED textures (see the header). On opaque content — which is what a
// node mid-graph is realistically fed — the two are bit-identical, so the
// bodies are still shared verbatim. On transparent content a node will show the
// straight-space fringe the edge path no longer has. Fixing it properly means
// premultiplying at sample time, which can't be done from the prelude alone
// (the bodies call texture() directly and GLSL ES forbids overloading it), so
// it would take a per-node conversion pass in the DAG executor. Deferred until
// something actually needs a transparent TRANSITION_FX.
